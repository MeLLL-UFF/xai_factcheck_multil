...
def create_instance(title, relevant_passages, src_language, src_passage_idx, all_passages, num_negative_samples=5):
    ...
    print('neg passages', len(negative_passages))
    # sample negative
...
def process_passage(passage_path, lang):
    try:
        passage_id = os.path.basename(passage_path).split('.txt')[0]
        doc_id = passage_id.split('-')[0]

        with open(passage_path, 'rb') as file:
            detector = charamel.Detector()
            result = detector.detect(file.read())
            print("encoding data:", result.value)
            file.close()

        title, passage = open(passage_path,'r', encoding=result.value).read().split('<sep>')
        ...
        print(f"Error processing {passage_path}: {e}")
        return None
...
def main(args):
    ...
    for lang in target_languages:
        print("start process")
        passage_paths = glob(os.path.join(args.passage_dir, f'{lang}/*.txt'))
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(process_passage, path, lang): path for path in passage_paths}
            for future in tqdm(as_completed(futures), total=len(futures)):
                result = future.result()
                if result:
                    all_passages.append(result)
                    doc_id = result['doc_id']
                    if doc_id not in doc_id2passages:
                        doc_id2passages[doc_id] = []
                    doc_id2passages[doc_id].append(result)

    print('passages len', len(all_passages))
    all_doc_ids = list(doc_id2passages.keys())
    print('all docs', len(all_doc_ids))
    shard_size = int(len(all_doc_ids) / int(args.num_shards))
    ...
    with open(args.out_file + f'_{args.shard_id}.tsv', 'w') as f:
