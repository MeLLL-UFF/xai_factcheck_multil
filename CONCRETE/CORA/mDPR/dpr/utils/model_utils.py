...
def add_missing_position_ids(state_dict, max_position_embeddings=512, hidden_size=768):
    position_ids = torch.arange(max_position_embeddings).unsqueeze(0).expand(1, -1).long()
    position_embeddings = nn.Embedding(max_position_embeddings, hidden_size)
    position_embeddings.weight.data.normal_(mean=0.0, std=0.02)
    
    state_dict['embeddings.position_ids'] = position_ids
    state_dict['embeddings.position_embeddings.weight'] = position_embeddings.weight.data
    return state_dict
