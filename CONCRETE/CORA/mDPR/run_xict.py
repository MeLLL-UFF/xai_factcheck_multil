...
class XICTBiEncoderTrainer(object):
    """
    ...
    def get_data_iterator(self, path: str, batch_size: int, shuffle=True,
        ...
        data = read_xict_samples_from_json_files(data_files, upsample_rates)
    ...
    def validate_nll(self) -> float:
        ...
        # Debug: Check if data_iterator is not None and iterate_data is callable
        if data_iterator is None or not hasattr(data_iterator, 'iterate_data') or not callable(data_iterator.iterate_data):
          logger.error("Data iterator is not properly initialized or iterate_data is not callable.")
          return None
        logger.info(data_iterator.__dict__)
        for i, samples_batch in enumerate(data_iterator.iterate_data()):
            ...
            loss, correct_cnt = _do_xict_fwd_pass(self.biencoder, biencoder_input, self.tensorizer, args)
    ...
    def _train_epoch(self, scheduler, epoch: int, eval_step: int,
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_data(epoch=epoch)):
            ...
            loss, correct_cnt = _do_xict_fwd_pass(self.biencoder, biencoder_batch, self.tensorizer, args)
...
def _do_xict_fwd_pass(model: nn.Module, input: BiEncoderBatch, tensorizer: Tensorizer, args) -> (
        torch.Tensor, int):
    input = BiEncoderBatch(**move_to_device(input._asdict(), args.device))
...
def main():
    ...
    trainer = XICTBiEncoderTrainer(args)
